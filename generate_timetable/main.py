...
if __package__:
    ...
    from .discover import discover_colours
    ...
    from generate_timetable.discover import discover_colours
...
def main():
    ...
    total_entries += count
    out_path = os.path.join("db", "timetable-computing.json")
    ...
    total_entries += count
    out_path = os.path.join("db", "timetable-business.json")
    ref_tt, written_count = write_json(tt, out_path)
    all_depts.update(ref_tt.keys())
    dlog(f"Wrote {out_path} ({written_count} entries, {len(ref_tt)} depts)")
    ...
    total_entries += count
    out_path = os.path.join("db", "timetable-engineering.json")
    ref_tt, written_count = write_json(tt, out_path)
    all_depts.update(ref_tt.keys())
    dlog(f"Wrote {out_path} ({written_count} entries, {len(ref_tt)} depts)")
